...
def calculate():
    display_state = display.get()
    try:
        math_expression = parser.expr(display_state).compiled()
...
Button(root, text="+", command= lambda:get_operation("+")).grid(row=2, column=4, sticky=W+E)
...
Button(root, text="/", command= lambda:get_operation("/")).grid(row=5, column=4, sticky=W+E)

Button(root, text="⇽", command= lambda: undo()).grid(row=2, column=4, sticky=W+E, columnspan = 2)
Button(root, text="exp", command= lambda:get_operation("exp")).grid(row=3, column=4, sticky=W+E)
